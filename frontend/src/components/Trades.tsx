...
const Trades: React.FC = () => {
    ...
    return (
      <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20 overflow-hidden">
        ...
        <div className="relative text-center">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">Error Loading Trades</h3>
    ...
    return (
      <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20 overflow-hidden">
        ...
        <div className="relative text-center">
          ...
          <p className="text-blue-200">Start trading to see your trade history.</p>
        </div>
      </div>
    );
  }
  ...
  const totalValue = tradesData.reduce((sum, trade) => sum + (trade.price * trade.qty), 0);
  const totalQuantity = tradesData.reduce((sum, trade) => sum + trade.qty, 0);
  const longTrades = tradesData.filter(trade => trade.side === 'LONG').length;
  const shortTrades = tradesData.filter(trade => trade.side === 'SHORT').length;
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          <div>
            ...
            <p className="text-blue-200">
              {totalTrades} trade{totalTrades !== 1 ? 's' : ''} • Execution history
            </p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-white/5 border-b border-white/20">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-medium text-blue-200 uppercase tracking-wider">Trade ID</th>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium text-blue-200 uppercase tracking-wider">Trade Value</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-blue-200 uppercase tracking-wider">Time</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/10">
                {tradesData.map((trade: Trade, index: number) => (
                  <motion.tr
                    key={trade.trade_id}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${getSideColor(trade.side)}`}>
                        {trade.side}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-200">
